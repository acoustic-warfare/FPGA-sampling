...
# Extract a portion of the impulse response around the largest peak
   truncated_impulse_response = fft_IR[max_index-450:max_index+600]
...
if __name__ == '__main__':
   ...
   plt.subplot(3,1,1)
   plt.plot(freqs[:fft_size//2], 20 *np.log(ref_mic_fft)[:fft_size//2],label="mic Freq spectrum")
   ...
   plt.subplot(3,1,3)
   plt.plot(freqs[:fft_size//2], 20 *np.log(ref_mic_FR)[:fft_size//2],label="Frequency response Mic")
   plt.xlabel('Frequency (Hz)')
   plt.ylabel('Magnitude')
   plt.legend(loc='upper right')
   plt.show()
   ...
   plt.subplot(2,1,1)
   plt.plot(other_mic,color='blue',label='other mic before')
   plt.xlabel('Samples')
   plt.ylabel('Amplitude')
   plt.legend(loc='upper right')

   plt.subplot(2,1,1)
   plt.plot(ref_mic,color='orange')
   plt.xlabel('Samples')
   plt.ylabel('Amplitude')
   plt.legend(loc='upper right')
