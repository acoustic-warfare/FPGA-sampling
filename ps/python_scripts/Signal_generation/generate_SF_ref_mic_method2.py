...
if __name__ == '__main__':
   ...
   ref_freq_resp = np.abs(ref_mic_fft) / np.abs(chirp_signal_fft)
   ...
   for i in range(0,64):
      ...
      other_freq_resp = np.abs(other_mic_fft) / np.abs(chirp_signal_fft)
   ...
   np.save('method2_short_d_log_high_3.npy', scaling_factor_array) # save the scaling factors to a file
