...
def int_to_twos_complement_string(num):
    if num >= 0:
        # Convert to binary string excluding the '0b' prefix
        binary = bin(num)[2:]
    ...
    # Remove space between last two bytes
...
if (txtInput.lower() == "y"):
    ...
    with open(fileChooser+".txt", "w") as f:
        while time.time() < t_end:
            ...
            f.write("   sampelCounter: " +
                    int_to_twos_complement_string_16bit(d.sampelCounter) + "        ")

            # Printing each mic data as integers
            for i in range(1, 200):
                field_name = f"mic_{i}"
                mic_data = int_to_twos_complement_string(
                    getattr(d, field_name))
                f.write(field_name + ": ")
                f.write(mic_data + "    ")
            f.write("\n")
