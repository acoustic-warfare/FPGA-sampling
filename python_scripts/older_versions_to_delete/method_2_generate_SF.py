...
if __name__ == '__main__':
   ...
   #chirp_signal *= (2**20-1)
   chirp_signal = chirp_signal/45 

   #get IR and FR for other mic
   chirp_IR = np.convolve(chirp_signal,matched_filter,mode='same')
   chirp_IR= truncation(chirp_IR)
   chirp_FR = np.fft.fft(chirp_IR,fft_size)
   ...
   np.save('method2_tukey_bit.npy', scaling_factor_array) # save the scaling factors to a file
