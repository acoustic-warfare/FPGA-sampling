...
if __name__ == '__main__':
   ...
   ref_mic_IR = np.fft.ifft(ref_mic_FR,fft_size)  #___________________________________________________________
   ...
   for i in range(0,64):
      ...
      mic_fft = np.fft.fft(mic,fft_size)

      mic_FR = mic_fft/chirp_fft

      mic_IR = np.fft.ifft(mic_FR,fft_size)  #______________________________________________
      mic_IR = truncation(mic_IR)
      mic_FR = np.fft.fft(mic_IR,fft_size)

      scaling_factor=ref_mic_FR/mic_FR
      scaling_factor = scaling_factor * np.exp(1j * np.zeros_like(scaling_factor))

      
      scaling_factor_array[i, :] =scaling_factor
   np.save('test.npy', scaling_factor_array) # save the scaling factors to a file
