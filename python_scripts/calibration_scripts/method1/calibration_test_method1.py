...
if __name__ == '__main__':
   ...
   other_mic_error = scaling_factor  #other_mic[0:fft_size] / other_mic_calibrated
...
#
   ...
   plt.subplot(3,1,2)
   plt.plot(other_mic[0:fft_size],label="MK16 before calibration",color="orange")
   plt.xlabel('Samples')
   plt.ylabel('Amplitude')
   plt.legend(loc='lower left')
...
#
   ...
   #plt.subplot(3,1,3)
   plt.plot(freqs[:fft_size//2], 20 *np.log(scaling_factor)[:fft_size//2],label="Deviation",color='red')
   plt.xlabel('Frequency (Hz)')
   plt.ylabel('Magnitude')
   plt.legend(loc='lower left')
   plt.show()
   ...
   # Plot the magnitude spectrum
   freqs = np.fft.fftfreq(fft_size, 1/fs)
   plt.plot(freqs[:fft_size//2], 20 *np.log(magnitude_spectrum_other_mic_calibrated_fft[:fft_size//2]),label="calibrated")
   plt.xlabel('Frequency (Hz)')
   plt.ylabel('Magnitude')
   plt.legend(loc='upper right')
   ...
   # Plot the magnitude spectrum
   freqs = np.fft.fftfreq(fft_size, 1/fs)
   plt.plot(freqs[:fft_size//2], 20 *np.log(magnitude_spectrum_other_mic_error[:fft_size//2]), label="deviation")
   plt.xlabel('Frequency (Hz)')
   plt.ylabel('Magnitude')
   plt.legend(loc='upper right')
   plt.tight_layout()
